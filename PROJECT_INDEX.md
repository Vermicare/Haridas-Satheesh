# Master Project Index

This index consolidates the projects, systems, prototypes, research programs and architectures developed across my professional work and independent R&D.

## Enterprise AI, EPMO & Governance
- CORTEX
- Governance Meeting Intelligence Layer
- Governance Memory Graph
- Governance Memory Mesh
- Governance Shadow Graph
- Organizational Gravity Mapping
- Federated Planner + Central Governance Intelligence
- Operational Digital Twin
- Governance Pulse Architecture
- Enterprise Decision Nervous System (EDNS)
- Enterprise Cognitive Operating System (ECOS)
- Project Nervous System
- Organizational Brain / Enterprise Memory System
- AI Meeting Brain
- MoM Approval & Distribution Automation
- Our Core PM System — FY26
- Standard Project Plans
- Project Hub
- Project Control Logs
- Program Health Updates
- Value Realization Tracking
- Delivery Calendar & Reminders
- EPMO Automation / Power Platform / Portfolio Analytics / AI Agents
- Uploaded Intelligence Status

## Climate-Tech & Personal Cooling
- Adaptive Non-AC Personal Cooling System
- Localized Dry-Core Cooling Pod
- Comfort Halo Loop
- Thermal Comfort Bubble
- Comfort Cone
- Adaptive Microclimate Platform
- Distributed Personal Thermal Ecosystem
- AirWell Tower
- Thermal Habitat Operating System (THOS)
- Localized Atmospheric Habitat System
- Dual-Zone Atmospheric Cocoon
- Nomadic / Resilient / Modular Atmospheric Habitat
- Comfort Persistence Simulation Model
- Predictive Comfort Ecology

## Retail Prediction & Analytics
- Retail Demand Intelligence
- Demand Opportunity Radar v0.1
- Feature Factory
- Derived Signal Laboratory
- Probabilistic Forecasting
- Stockout-adjusted Demand
- Model Tournament
- Opportunity / Financial Layer
- Experiment Registry
- Human-in-the-loop Retail Execution

## Personal Intelligence Software
- Companion Intelligence — Life OS
- Minimum Viable Companion
- Kingdom Graph
- Daily Brief
- Weekly Kingdom Review
- Life Seasons Guided Journey
- First-run Onboarding
- Standalone Tester
- Specialist Agents
- Spider Execution Engine
- Career & Income Decision Workspace
- AI Architect Agent
- Evidence Conflict Detection
- Explicit Supersession
- Approved-Claim Decision Versioning
- Knowledge Inbox
- Explicit Claim Approval
- Read-only Knowledge Source Boundary
- Google Drive Knowledge Adapter
- Knowledge Graph Workstreams
- Recommendation Engines
- Dashboards / Personal Intelligence Views
- SQLCipher Research Probe
- Production Encrypted Store Evaluation
- Atomic Record / Receipt Transactions
- Linux Secret Service Key-Custody Probe
- Cross-platform Packaging & Interaction Simulation
- Constitution / System Specification
- Architecture Decision Records
- Roadmap & Execution Workflows

## Human / Health Operating-System Concepts
- Human OS
- Health OS
- Athlete OS
- Personal OS Health Protocol
- Personal Health / Adventure System

## Hardware & IoT
- Raspberry Pi Environmental Sensing Platform
- ESP32-S3 Sensor Nodes
- CO₂ / Environmental Monitoring
- Thermal Camera Experimentation
- Instrumentation & Electronics Bench
- AI-based Soil Health Monitor
- Raspberry Pi / ChatGPT Smart Speaker

## Health & Human Performance
- Joint, Synovial & Connective-Tissue Resilience
- 12-week Tissue / Exercise Protocol
- Exercise Protocol Research Program
- Personal OS Health Protocol
- Personal Health / Adventure System

## Sustainability & Social Impact
- VermiCare
- AI-based Soil Health Monitor
- Design for Change / #MakeAChange

## Career & Growth Systems
- Career OS
- Global Job Search System
- Freelance Opportunity Manager
- Global Operations Leads Workflow
- Personal Brand / Build-in-Public System

## Professional Delivery
- AI-Gemini Predictive Tracking
- LLM Training & QA Operations System
- BYJU'S Agile Delivery & Workflow Automation
- Enterprise WebEx / Network Migration Delivery
- Sinclair + Verizon SD-WAN / IPCC Support
- VoIP Migration / Porting / Webex Workstreams
- CPE / MLAN / MPLS / VNS / Managed Services Delivery
- Verizon Business Optical Rings Support
- Jira / Asana / Trello / Microsoft Project Delivery Tooling
- Odoo Dashboard / Functionality Customization & Team Operations
- B2B Dashboard / UI-UX Design Work
- JCPenney / Catalyst Brands EPMO Microsoft 365 Automation Portfolio

## Venture & Product Explorations
- Enterprise Decision & Action Intelligence Platform
- AI-native Enterprise Operations Intelligence
- AI Transformation Operating Layer for Microsoft-heavy mid-market companies
- PMO Portfolio Intelligence Startup Wedge
- Decision Memory / Organizational Memory Market Study
- Global Innovation Company Thesis

## Creative / Knowledge Transformation
- Hidden Patterns of Being Human — Gentle Edition
